...
public partial class OkulSinavi_CevrimiciSinavYonetim_OgrenciKarne : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            TestOgrPuanInfo ogrCevap = null;
            if (kInfo.Yetki.Contains("OkulYetkilisi"))
            {
                ogrCevap = testOgrCevapDb.KayitBilgiGetir(sinavId, kInfo.KurumKodu.ToInt32(), opaqId);
            }
            else if (kInfo.Yetki.Contains("IlceMEMYetkilisi"))
            {
                ...
                ogrCevap = testOgrCevapDb.KayitBilgiGetir(sinavId, ilce.IlceAdi, opaqId);
            }
            else if (kInfo.Yetki.Contains("Root") || kInfo.Yetki.Contains("Admin") || kInfo.Yetki.Contains("LgsIlKomisyonu"))
            {
                ogrCevap = testOgrCevapDb.KayitBilgiGetir(sinavId, opaqId);
            }
