...
using MySql.Data.MySqlClient;
    ...
    public class CkKarneKutukDB
    {
        ...
        public DataTable KayitlariGetirGecici()
        {
            const string sql = "select * from ckkarnekutuk where KitapcikTuru<>'-' order by Id asc";
            return helper.ExecuteDataSet(sql).Tables[0];
        }
        ...
        public List<CkKarneKutukInfo> KayitlariDizeGetir(int sinavId, int kurumKodu)
        {
            ...
            foreach (DataRow k in dt.Rows)
            {
                karne.Add(new CkKarneKutukInfo(Convert.ToInt32(k["Id"]), k["OpaqId"].ToString(), k["IlceAdi"].ToString(), 
        ...
        public void KayitGuncelle(int id,string kitapcikTuru,string cevaplar)
        {
            const string sql = @"update ckkarnekutuk set KitapcikTuru=?KitapcikTuru,Cevaplar=?Cevaplar where Id=?Id";
            MySqlParameter[] pars =
            {
                new MySqlParameter("?KitapcikTuru", MySqlDbType.String),
                new MySqlParameter("?Cevaplar", MySqlDbType.String),
            ...
            pars[0].Value = kitapcikTuru;
            pars[1].Value = cevaplar;
            pars[2].Value = id;
            helper.ExecuteNonQuery(sql, pars);
        }
        ...
        public int KalanKayitSayisi()
        {
            string sql = @"select COUNT(Id) from ckkarnekutuk where KitapcikTuru<>'-'";

            int ogrenciSayisi = Convert.ToInt32(helper.ExecuteScalar(sql));
            return ogrenciSayisi;
        }
    }
