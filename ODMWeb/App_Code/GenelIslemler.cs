    ...
    public static void CookieSil(params string[] s)
    {
        try
        {
            foreach (string s2 in s)
            {
                ...
                {
                    HttpCookie myCookie = new HttpCookie(s2) {Expires = DateTime.Now.AddDays(-1d)};
...
//
    ...
    {
        TimeSpan ts = DateTime.Now - Convert.ToDateTime(tarih);
        return ts.Days.ToString();
    }
    public static String KalanGun(this DateTime tarih)
    {
        TimeSpan ts = Convert.ToDateTime(tarih) - DateTime.Now;
